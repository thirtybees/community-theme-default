        ...
        <div class="product clearfix col-xs-12 col-sm-6">
          <div class="thumbnail" style="width: {$homeSize.width|intval}px; height: {$homeSize.height|intval}px">
            ...
            <picture class="img-responsive{if !empty($lazy_load)}tb-lazy-image{/if}" style="width: {$homeSize.width|intval}px; height: {$homeSize.height|intval}px">
              ...
              {if !empty($webp)}
                <source {if !empty($lazy_load)}srcset="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs= 1w" data-{/if}srcset="{$link->getImageLink($stf_product->link_rewrite, $stf_product_cover, 'home_default', 'webp', ImageManager::retinaSupport())|escape:'html':'UTF-8'}"
                        type="image/webp"
                >
              {/if}
              <!--[if IE 9]></video><![endif]-->
              <img {if !empty($lazy_load)}srcset="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs= 1w" data-{/if}srcset="{$link->getImageLink($stf_product->link_rewrite, $stf_product_cover, 'home_default', null, ImageManager::retinaSupport())|escape:'html':'UTF-8'}"
