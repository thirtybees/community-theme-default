          ...
          <dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}" class="clearfix">
            <a class="cart-images" href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">
              ...
              <picture {if !empty($lazy_load)}class="tb-lazy-image"{/if}>
                ...
                {if !empty($webp)}
                <source {if !empty($lazy_load)}srcset="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs= 1w" data-{/if}srcset="{$link->getImageLink($product.link_rewrite, $product.id_image, 'cart_default', 'webp', ImageManager::retinaSupport())}"
                        alt="{$product.name|escape:'html':'UTF-8'}"
                        type="image/webp"
                >
                {/if}
                <!--[if IE 9]></video><![endif]-->
                <img {if !empty($lazy_load)}srcset="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs= 1w" data-{/if}srcset="{$link->getImageLink($product.link_rewrite, $product.id_image, 'cart_default', null, ImageManager::retinaSupport())}"
