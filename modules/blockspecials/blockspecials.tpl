                        ...
                        <article>
                            <a class="products-block-image" href="{$special.link|escape:'html':'UTF-8'}"
                              ...
                              <picture class="img-responsive{if !empty($lazy_load)} tb-lazy-image{/if}">
                                ...
                                {if !empty($webp)}
                                  <source {if !empty($lazy_load)}srcset="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs= 1w" data-{/if}srcset="{$link->getImageLink($special.link_rewrite, $special.id_image, 'small_default', 'webp', ImageManager::retinaSupport())|escape:'html':'UTF-8'}"
                                          title="{$special.name|escape:'html':'UTF-8'}"
                                          type="image/webp"
                                  >
                                {/if}
                                <!--[if IE 9]></video><![endif]-->
                                <img {if !empty($lazy_load)}srcset="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs= 1w" data-{/if}srcset="{$link->getImageLink($special.link_rewrite, $special.id_image, 'small_default', null, ImageManager::retinaSupport())|escape:'html':'UTF-8'}"
